...
//	}
	...
	public static int dup(int paramInt) {
	...
	private class RWorker extends HackThread {
		...
		private void gotack(int seq) {
			synchronized (pending) {
				for (ListIterator<Message> i = pending.listIterator(); i
						.hasNext();) {
					Message msg = i.next();
					if (msg.seq <= seq)
						i.remove();
			...
			while (msg.off < msg.blob.length) {
				...
				synchronized (oc) {
					while (true) {
						...
						if (type == OD_REM) {
							oc.remove(id, frame);
							//System.out.println("OD_REM " + frame);
							if(!Config.soundMemo) Sound.soundSet.remove(id);
						} else if (type == OD_MOVE) {
							Coord c = msg.coord();
							oc.move(id, frame, c);
							//System.out.println("OD_MOVE (frame: " + frame + ", id: " + id + "): coord: " + c.toString());
						} else if (type == OD_RES) {
							int resid = msg.uint16();
							Message sdt;
							boolean flagln = false;
							if ((resid & 0x8000) != 0) {
								flagln = true;
								resid &= ~0x8000;
								int ln = msg.uint8();
								//System.out.println("lnDerive = " + ln);
								sdt = msg.derive(0, ln/*msg.uint8()*/);
							} else {
								sdt = new Message(0);
							}
							if (flagln) {
								//System.out.println("OD_RES");
							...
							int dist = (int) (s.dist(t));
							//System.out.println("OD_LINBEG (frame: " + frame + ", id: " + id + "): steps: " + c + ", s: " + s.toString() + ", f: " + t.toString() + ", dist: " + dist);
						} else if (type == OD_LINSTEP) {
							int l = msg.int32();
							oc.linstep(id, frame, l);
							//System.out.println("OD_LINSTEP (frame: " + frame + ", id: " + id + "): step: " + l);
						} else if (type == OD_SPEECH) {
							//System.out.println("OD_SPEECH");
							Coord off = msg.coord();
							//System.out.println("off = " + off.toString());
							...
							if (type == OD_LAYERS) {
								int bid = msg.uint16();
								baseres = getres(/*msg.uint16()*/bid);
								//System.out.println("OD_LAYERS " + frame);
								//System.out.println("baseres = " + baseres.get().name);
								//System.out.println("baseres = " + bid);
							}
							...
							if (type == OD_LAYERS) {
								...
								//System.out.println("OD_AVATAR");
							}
						} else if (type == OD_DRAWOFF) {
							//System.out.println("OD_DRAWOFF");
							Coord off = msg.coord();
							//System.out.println("off = " + off.toString());
							...
							int sz = msg.uint16();
							int str = msg.uint8();
							//System.out.println("OD_LUMIN (id = " + id + "): off = " + off.toString() + ", sz = " + sz + ", str = " + str);
							oc.lumin(id, frame, off, sz, str);
							...
							//System.out.println("OD_FOLLOW: oid: " + oid + ", szo: " + szo + ", off: " + off.toString());
							oc.follow(id, frame, oid, off, szo);
						} else if (type == OD_HOMING) {
							int oid = msg.int32();
							if (oid == -1) {
								oc.homostop(id, frame);
								//System.out.println("OD_HOMING (frame: " + frame + ", id: " + id + ")");
							} else {
								Coord tgtc = msg.coord();
								int v = msg.uint16();
								//System.out.println("OD_HOMING (id: " + id + "): oid: " + oid + ", v: " + v);
								if (oid == -2) {
							...
							//System.out.println("OD_OVERLAY, id: " + id);
							int olid = msg.int32();
							...
							if (resid == 65535) {
								...
								if ((resid & 0x8000) != 0) {
									...
									/*if (resid == 7438) {
										Message sdtc = sdt.clone();
										int s = sdtc.int32();
										sdtc.uint8();
										int k = sdtc.uint16();
										Color c = new Color(dup((k & 0xF000) >> 12), dup((k & 0xF00) >> 8), dup((k & 0xF0) >> 4), dup((k & 0xF) >> 0));
										
										System.out.println("SCORE: time: " + System.currentTimeMillis() + ", frame: " + frame + ", score: " + s + ", color16: " + c.toString() + " alpha: " + c.getAlpha());
									}*/
								} else {
									sdt = new Message(0);
								}
								res = getres(resid);
								/*try {
									System.out.println("res = " + res.get().name);
									System.out.println("resid = " + resid);
								} catch (Exception e) {
									e.printStackTrace();
								}*/
							...
							int hp = msg.uint8();
							//System.out.println("OD_HEALTH: " + hp + ", frame: " + frame + ", id: " + id);
							oc.health(id, frame, hp);
						} else if (type == OD_BUDDY) {
							//System.out.println("OD_BUDDY");
							...
							//System.out.println("OD_END");
							//System.out.println("-----------------");
							break;
						} else {
							throw (new MessageException(
									"Unknown objdelta type: " + type, msg));
						}
					}
		...
		private void handlerel(Message msg) {
			if (msg.type == Message.RMSG_NEWWDG) {
				...
				//if (resname.contains("gfx/arch") || resname.contains("gfx/borka") || 
				//resname.contains("gfx/kritter") || resname.contains("gfx/terobjs"))
					//System.out.println("RESID(" + resid + ", " + resname + ")");
				int resver = msg.uint16();
				synchronized (rescache) {
					getres(resid).set(Resource.load(resname, resver, -5));
				}
				Config.addRes(resid, resver, resname);  // res id project
				...
				Audio.play(res);
				Resource r = res.get();
				//System.out.println("Sound: res: " + (r != null ? r.name : null) + ", time: " + System.currentTimeMillis());
		...
		public void run() {
			try {
				...
				while (alive) {
					DatagramPacket p = new DatagramPacket(new byte[65536],
							65536);
					try {
						sk.receive(p);
					...
					if (!p.getAddress().equals(server))
						continue;
					Message msg = new Message(p.getData()[0], p.getData(), 1,
							p.getLength() - 1);
					...
					if (state != "conn") {
						if (msg.type == MSG_SESS) {
							...
							while (!msg.eom()) {
								...
								getrel(seq, new Message(type, msg.blob,
										msg.off, len));
								msg.off += len;
								seq++;
							}
							...
							// throw(new
							// MessageException("Unknown message type: " +
							// msg.type, msg));
			...
			try {
				...
				while (true) {
					...
					if (state == "conn") {
						...
						synchronized (objacks) {
							Message msg = null;
							for (Iterator<ObjAck> i = objacks.values()
									.iterator(); i.hasNext();) {
								ObjAck a = i.next();
