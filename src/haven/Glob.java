...
public class Glob {
	...
	public Glob(Session sess) {
		...
		paginae.add(Resource.load("paginae/add/fep")); // new
	}
	...
	public void cattr(Message msg) {
		synchronized (cattr) {
			while (!msg.eom()) {
				...
				/*if (Arrays.asList("hp", "fxfer", "idlstam", "regen", "fepmod").contains(nm)) {
					System.out.println(nm + ": base: " + base + ", comp: " + comp);
				}*/ // some weird attrs that loftar's server sends
			}
		}
	}
	...
	public void buffmsg(Message msg) {
		...
		synchronized (buffs) {
			if (name == "clear") {
				...
				//System.out.println("time: " + System.currentTimeMillis() + ", buffmsg: " + name);
			} else if (name == "set") {
				int id = msg.int32();
				...
				Resource resnm = res.get();
				//System.out.printf("time: %d, buffmsg: %s, res: %s, id: %d, tt: %s, ameter: %d, nmeter: %d, cmeter: %d, cticks: %d, major: %d\n", System.currentTimeMillis(), name, (resnm == null || resnm.name == null ? "" : resnm.name), id, tt, ameter, nmeter, cmeter, cticks, (major ? 1 : 0));
			} else if (name == "rm") {
				int id = msg.int32();
				buffs.remove(id);
				
				//System.out.println("time: " + System.currentTimeMillis() + ", buffmsg: " + name + ", id: " + id);
			}
		}
	}
}
