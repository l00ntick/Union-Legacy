...
public class Inventory extends Widget implements DTarget {
	...
	/*if (name.contains("Bucket") && !name.contains("Empty"))
		String fknbug = "bucket-"+name.split(" ")[6].replace(")", "");
	else 
		String fknbug = "1234567890";*/
	for (Widget wdg = lchild; wdg != null; wdg = wdg.prev) {
	    if (wdg.visible && wdg instanceof Item) {
			if (((Item) wdg).name().equals(name)/* || ((Item) wdg).GetResName().contains(fknbug)*/)
				items.add((Item) wdg);
		}
	}
