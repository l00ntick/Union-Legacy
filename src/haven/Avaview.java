...
public class Avaview extends Widget {
	...
	public Avaview(Coord c, Widget parent, List<Indir<Resource>> rl) {
		...
		if (rl.size() == 0)
			...
			this.myown = new AvaRender(rl); // renders avatar at logscreen
	}
	...
	public void draw(GOut g) {
			...
			if (Config.showMyAvatar) {
				...
				if (gob != null)
					if (UI.instance.mainview != null)
						if (UI.instance.mainview.playergob != gob.id)
							ava = gob.getattr(Avatar.class);
				if (ava != null)
					at = ava.rend;
			}
		...
		if (at == null) {
			...
			g2.image(Equipory.bg, new Coord(Equipory.bg.sz().x / 2 - asz.x / 2,
					20).inv());
			yo = (20 * asz.y) / dasz.y;
		}
		Coord tsz = new Coord((at.sz().x * asz.x) / dasz.x, (at.sz().y * asz.y)
				/ dasz.y);
		g2.image(at, new Coord(tsz.x / 2 - asz.x / 2, yo).inv(), tsz);
		g.chcolor(color);
		Window.wbox.draw(g, Coord.z,
				asz.add(Window.wbox.bisz()).add(unborder.mul(2).inv()));
		Tex name;
		if(((name = name()) != null)){
