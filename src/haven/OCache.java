...
public class OCache implements Iterable<Gob> {
    ...
    public synchronized void overlay(int id, int frame, int olid, boolean prs, Indir<Resource> resid, Message sdt) {
    	...
    	if(resid != null) {
			...
			}/* else if (resid.get().name.equals("gfx/fx/fire")) {
				Message paramMessage = sdt.clone();
				int i = paramMessage.uint8();
				int j = paramMessage.uint8();
				int intns = paramMessage.uint8();
				int fadepow = paramMessage.uint8();
				int life = paramMessage.uint8();
				int inityv = paramMessage.uint8();
				System.out.printf("gobid: %d, params(uint8): i: %d, j: %d, intns: %d, fadepow: %d, life: %d, inityv: %d\n", id, i, j, intns, fadepow, life, inityv);
			} else if (resid.get().name.equals("gfx/fx/ismoke")) {
				Message paramMessage = sdt.clone();
				int i = paramMessage.uint8();
				int j = paramMessage.uint8();
				int col = paramMessage.uint16();
				int fadepow = paramMessage.uint8();
				int life = paramMessage.uint8();
				int inityv = paramMessage.uint8();
				System.out.printf("gobid: %d, params(uint8): i: %d, j: %d, col(uint16): %d, fadepow: %d, life: %d, inityv: %d\n", id, i, j, col, fadepow, life, inityv);
			}*/
			}catch(Exception e){}
    		if(ol == null) {
