...
public class Makewindow extends HWindow {
	...
	public Makewindow(Widget parent, String rcpnm) {
		...
		nm.setcolor(Color.black); //Kerri
		craft_name = rcpnm;
		nm.c = new Coord(sz.x - 10 - nm.sz.x, 10);
		Label iLabel = new Label(new Coord(10, 18), this, "Input:");
		Label rLabel = new Label(new Coord(10, 73), this, "Result:");
		iLabel.setcolor(Color.black); //Kerri
	...
	public void uimsg(String msg, Object... args) {
		if (msg == "pop") {
			synchronized (this) {
				...
				inputItems = new LinkedList<Item>();
				outputItems = new LinkedList<Item>();
				int i;
				...
				for (i = 0; (Integer) args[i] >= 0; i += 2) {
					...
					inputItems.add(new Item(Coord.z, (Integer) args[i], -1, box, null,
							(Integer) args[i + 1]));
				}
				c = new Coord(xoff, 65);
				for (i++; (i < args.length) && ((Integer) args[i] >= 0); i += 2) {
					...
					outputs.add(box);
					c = c.add(new Coord(31, 0));
					outputItems.add(new Item(Coord.z, (Integer) args[i], -1, box, null,
							(Integer) args[i + 1]));
				}
			}
		}
	...
	public synchronized List<Item> getInputItems() {
		if (inputItems != null) {
			return new LinkedList<Item>(inputItems);
		}

		return null;
	}

	public synchronized List<Item> getOutputItems() {
		if (outputItems != null) {
			return new LinkedList<Item>(outputItems);
		}
