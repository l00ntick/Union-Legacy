...
public class MapView extends Widget implements DTarget, Console.Directory {
	...
	//place res sdt
	public static Map<String, Integer> placeResSdts = new HashMap<String, Integer>();
	...
	static {
		...
		placeResSdts.put("gfx/terobjs/furniture/skullthrone", 2);
		placeResSdts.put("gfx/terobjs/cupboard", 1);
		placeResSdts.put("gfx/terobjs/lchest", 1);
		placeResSdts.put("gfx/terobjs/furniture/chair", 2);
		placeResSdts.put("gfx/kritter/wagon/wagon-1", 1);
		placeResSdts.put("gfx/terobjs/cauldron", 33); //clay
		placeResSdts.put("gfx/terobjs/cauldron", 64); //metal (same res, need to check last sent command)
		placeResSdts.put("gfx/terobjs/furniture/coffer", 1);
		placeResSdts.put("gfx/terobjs/crate", 1);
		placeResSdts.put("gfx/terobjs/birchbasket", 1);
		placeResSdts.put("gfx/terobjs/furniture/wardrobe", 1);
		placeResSdts.put("gfx/terobjs/furniture/carpet", 1);
		placeResSdts.put("gfx/terobjs/furniture/bigchest", 1);
		placeResSdts.put("gfx/terobjs/fforge", 1);
		placeResSdts.put("gfx/terobjs/smelter", 1);
		placeResSdts.put("gfx/terobjs/sbox", 1);
		placeResSdts.put("gfx/terobjs/coinpress", 1);
	}
	...
	private static Camera restorecam() {
		Class<? extends Camera> ct = camtypes.get(Utils.getpref("union.defcam",
				"border"));
		if (ct == null)
	...
	public static Coord viewoffset(Coord sz, Coord vc) {
	...
	public boolean pathfinder(int button, Gob hit, Coord c, int modflag){
		if (ui.menugrid.pathfindAction != null && Config.pathfinder) {
			if (!ui.m_util.getCursor().endsWith("arw")) {
				wdgmsg("click", JSBotUtils.getCenterScreenCoord(), ui.m_util.getPlayerCoord(), 3, 0);
			}
		}
		...
		if((ui.modflags() == 3 || Config.pathfinder) && (button == 1 || button == 3 || button == 10)){
			...
			if((button == 1 || button == 3) && plob != null ){
				...
				if(button == 3){
					wdgmsg("place", plb.rc, button, modflag);
					return true;
				}
				...
				//if(ui.m_util.boxFree(mousepos, plb) ){
					...
					walk.start();
				//}
			}else if((button == 1 || button == 3) && ui.menugrid.pathfindAction != null ){
				if(button == 3){
					wdgmsg("click", Coord.z, Coord.z, 3, 0);
					ui.menugrid.pathfindAction = null;
					return true;
				}
				...
				if(hit != null){
					...
					walk.m_action = ui.menugrid.pathfindAction;
					walk.start();
					ui.menugrid.pathfindAction = null;
				}else{
					ui.m_util.PFrunning = true;
				...
				//if(ui.m_util.boxFree(ui.modshift?tilify(mousepos):mousepos, lifted) ){
					ui.m_util.PFrunning = true;
					ui.m_util.pathing = true;
					ui.m_util.stop = false;
					walk = new PathWalker( ui.m_util, ui.modshift?tilify(mousepos):mousepos);
	...
	boolean boatTest(){
		Gob boat = ui.m_util.findClosestObject("boat");
		Gob player = ui.m_util.getPlayerGob();
		if(boat == null) return false;
		return (ui.m_util.checkPlayerSitting() && player.getc().equals(boat.getc()) );
	}
	...
	public boolean mousedown(Coord c, int button) {
		...
		if (ui.menugrid.pathfindAction != null) {
			pathAction = ui.menugrid.pathfindAction;
			ui.menugrid.pathfindAction = null;
		} else {*/
		if (button != 1 || !ui.modctrl || !boatTest()) {
			if (hit != null && hit.resname().contains("gfx/terobjs/herbs/flotsam") && button == 3) {
				if (pathfinder(1, null, c, modflag)) {
					wdgmsg("click", c0, mc, button, ui.modflags(), hit.id, hit.position());
					return true;
				}
				//if(pathfinder(button, hit, c, modflag)) return true;
			} else if (pathfinder(button, hit, c, modflag))
				return true;
		}
		//} 

		/*if(pathfinder(3, hit, c, modflag)) {
			JSBotUtils.mapAbsClick(0, 0, 1, 0);
			ui.menugrid.wdgmsg("act", (Object[]) pathAction);
			wdgmsg("click", hit.getr(), mc, 1, ui.modflags(), hit.id, hit.position());
			return true;
		}*/
		...
		if ((cam != null) && cam.click(this, c, mc, button)) {
			...
			for (Gob g : plob)
				gob = g;
			ui.m_util.forceStop();
			wdgmsg("place", gob.rc, button, modflag);
		} else {
			//Kerri
			...
			//Kerri
			if(button == 1 && ui.modctrl && ui.modshift/* && !ui.modctrl*/) {
				if(hit != null)
			...
			if (hit == null) {
				if (button == 1 && modflag == 1/* && !ui.modctrl && !ui.modmeta*/) {
					glob.oc.enqueue(mc);
					glob.oc.checkqueue();
				} else {
					...
					ui.m_util.forceStop();
					wdgmsg("click", c0, mc, button, modflag);
				}
			} else {
				//Kerri: option
				...
				ui.m_util.forceStop();
				wdgmsg("click", c0, mc, button, modflag, hit.id,
						hit.position());
			}
	...
	public void uimsg(String msg, Object... args) {
		if (msg == "move") {
			...
			gob.setattr(new ResDrawable(gob, res, getSdtForPlaceRes(res.name)));
			plob.add(gob);
			glob.oc.ladd(plob);
			if (args.length > 3) {
	...
	private static Message getSdtForPlaceRes(String res) {
		Message msg = new Message(0);
		if (placeResSdts.containsKey(res)) {
			msg.adduint8(placeResSdts.get(res));
		}
		return msg;
	}
	...
	private void hearth() {
		...
		ui.menugrid.wdgmsg("act", (Object[])action);
		lastah = System.currentTimeMillis() + 5000;
	}

	/*private void drawBeastRadius(GOut g) {
		String name;
		g.chcolor(200, 0, 255, 96);
		synchronized (glob.oc) {
			for (Gob tg : glob.oc) {
				name = tg.resname();
				if ((tg.sc != null)
						&& (name.indexOf("/cdv") < 0)
						&& ((name.indexOf("kritter/boar") >= 0) || 
							(name.indexOf("kritter/bear") >= 0) || 
							(name.indexOf("kritter/bram") >= 0))) {
					drawradius(g, tg.sc, 100);
				}
                Sound.soundGobList(tg);
			}
		}
		g.chcolor();
	}
		...
		if (Config.showHidden && Config.hide) {
			...
			synchronized (glob.oc) {
				for (Gob gob : glob.oc) {
					...
					if (res != null && res.name != null && res.name.contains("gfx/arch/sign")) {
	...
	public void draw(GOut og) {
		...
		g.scale(getScale());
		checkmappos();
		//System.out.printf("mc = (%d, %d)\n", mc.x, mc.y);
		Coord requl = mc.add(-500, -500).div(tileSize).div(cmaps);
		Coord reqbr = mc.add(500, 500).div(tileSize).div(cmaps);
		//System.out.printf("requl = (%d, %d)\n", requl.x, requl.y);
		//System.out.printf("reqbr = (%d, %d)\n", reqbr.x, reqbr.y);
		Coord cgc = new Coord(0, 0);
		for (cgc.y = requl.y; cgc.y <= reqbr.y; cgc.y++) {
			for (cgc.x = requl.x; cgc.x <= reqbr.x; cgc.x++) {
				if (map.grids.get(cgc) == null)
				...
				if (args.length >= 2) {
					...
					if (ct != null) {
						try {
							...
							Utils.setpref("union.defcam", args[1]);
