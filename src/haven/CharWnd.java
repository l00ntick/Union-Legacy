...
public class CharWnd extends Window {
	...
	class NAttr extends Attr {
		...
		public void update() {
			try {
				...
				long SewDextPsy = ((long) sew.attr.comp * dex.attr.comp * psy.attr.comp);
				lblSewDexPsy.settext((int) (Math.cbrt(SewDextPsy)) + "");
	...
	public class FoodMeter extends Widget {
		...
		private Coord detsz = foodmimg.sz().sub(10, 5);
		private Coord detc = new Coord(-5, 0);
		...
		public void toggle(){
			if(wnd == null){
				wnd = new Window(new Coord(150, 150), detsz, ui.root, "FEP"){
					public void destroy(){
						wnd = null;
						if(!attached){FoodMeter.this.unlink();}
						super.destroy();
					}
				};
				wnd.justclose = true;
	...
	public class Worship extends Widget {
		...
		public Worship(Coord c, Widget parent, String title, Tex img) {
			...
			for (int i = 0; i < wishes.length; i++)
				wishes[i] = new Inventory(new Coord(i * 31 + 2, 119), new Coord(1, 1), this);
		...
		public void wish(int i, Indir<Resource> res, int amount) {
			wishes[i].unlink();
			wishes[i] = new Inventory(new Coord(i * 31 + 2, 119), new Coord(1, 1),
					this);
	...
	public class Study extends Widget {
		...
		public void onLAUpdate() { //if LA changes, then lp changes also
			upd();
		}
	}
	...
	public CharWnd(Coord c, Widget parent, int studyid) {
		...
		foodm = new FoodMeter(new Coord(10, 167), cattr);
	...
	public void uimsg(String msg, Object... args) {
		if (msg == "exp") {
			...
			for (int i = 0; i < args.length; i++) {
				...
				if(first){
					if(Config.toggleTR && name.equals("ranger")) {
						enableTracking();
					} else if(Config.toggleCA && name.equals("srv")) {
						enableCriminal();
					}
				}
	...
	public class sliderClass{
		...
		boolean pushSlider(){
			...
			if(value == (5 * push)){
				...
				ui.slenhud.error("Sliding towards " + getName(push));
				value = value + push;
	...
	void enableTracking(){
		ui.menugrid.wdgmsg("act", "tracking");
	}
	
	void enableCriminal(){
		ui.menugrid.wdgmsg("act", "crime");
	}
