...
public class SlenHud extends ConsoleHost implements DTarget, DropTarget,
        Console.Directory, IHWindowParent {
    ...
    public static final Color urgcols[] = {null, new Color(0, 128, 255),
    ...
    Button sub, sdb;
    VC vc;
    static Text.Foundry errfoundry = new Text.Foundry(new java.awt.Font(
            "SansSerif", java.awt.Font.BOLD, 14), new Color(192, 0, 0));
    Text lasterr;
    long errtime;
    OptWnd optwnd = null;
    static int dh;
    ...
    static {
        ...
        if (lowResNum > 1) {
            bg = Resource.loadtex("gfx/hud/slen/low" + lowResNum);
        }
    ...
    static class FoldButton extends IButton {
        ...
        public FoldButton(Coord c, Widget parent) {
            super(c, parent, Resource.loadimg("gfx/hud/slen/sbu"), Resource
                    .loadimg("gfx/hud/slen/sbd"));
            dy = sz.y;
        }

        public void draw(GOut g) {
            c.x = (MainFrame.getInnerSize().x - sz.x) / 2;
            c.y = MainFrame.getInnerSize().y + dy;
            super.draw(g);
    ...
    public SlenHud(Coord c, Widget parent) {
        super(new Coord((MainFrame.getInnerSize().x - sz.x) / 2,
                MainFrame.getInnerSize().y - sz.y), sz, parent);
        ui.slenhud = this;
        if (Config.new_chat)
            new ChatHWPanel(new Coord(0, MainFrame.getInnerSize().y - 300),
                    new Coord(350, 300), ui.root);
        else
            ui.chat = this;
        dy = -sz.y;
        ...
        hb = new IButton(mc, this, Resource.loadimg("gfx/hud/slen/hbu"),
                Resource.loadimg("gfx/hud/slen/hbd"));
		invb = new IButton(mc, this, Resource.loadimg("gfx/hud/slen/invu"),
				Resource.loadimg("gfx/hud/slen/invd"));
		equb = new IButton(mc, this, Resource.loadimg("gfx/hud/slen/equu"),
				Resource.loadimg("gfx/hud/slen/equd"));
		chrb = new IButton(mc, this, Resource.loadimg("gfx/hud/slen/chru"),
				Resource.loadimg("gfx/hud/slen/chrd"));
		budb = new IButton(mc, this, Resource.loadimg("gfx/hud/slen/budu"),
				Resource.loadimg("gfx/hud/slen/budd"));
		optb = new IButton(mc, this, Resource.loadimg("gfx/hud/slen/optu"),
				Resource.loadimg("gfx/hud/slen/optd"));
        if (!Config.new_minimap) {
            {
                new IButton(dispc, this,
        ...
        vc = new VC(this, fb = new FoldButton(new Coord(
                (MainFrame.getInnerSize().x - 40) / 2,
                MainFrame.getInnerSize().y), parent) {
            public void click() {
                vc.show();
            }
        });
        sub = new Button(new Coord(134, 29), 100, this,
                Resource.loadimg("gfx/hud/slen/sau")) {
            public void click() {
                sup();
            }
        };
        sdb = new Button(new Coord(134, 109), 100, this,
    ...
    public void uimsg(String msg, Object... args) {
        if (msg == "err") {
            ...
            if (args.length < 2) {
                ...
                ToolbarWnd.setbelt((Integer) args[0],
        ...
        if (wnd.title.equals("Messages")) {
            ui.cons.out = new java.io.PrintWriter(new java.io.Writer() {
                ...
                public void write(char[] src, int off, int len) {
                    ...
                    int p;
                    while ((p = buf.indexOf("\n")) >= 0) {
                        ((Logwindow) wnd).log.append((Config.timestamp ? Utils
                                .timestamp() : "") + buf.substring(0, p));
                        buf.delete(0, p + 1);
                    }
                }
    ...
    // public Indir<Resource> get_previous_bind(int slot) {
    // return belt[currentBelt][slot];
    // }
    //
    // public void bind_drag_item(int slot) {
    // wdgmsg("setbelt", slot, 0);
    // }
    //
    // public void bind_item(Indir<Resource> res, int slot) {
    // belt[currentBelt][slot] = res;
    // wdgmsg("setbelt", slot, res.get().name);
    // }

    public void activate_bind(int slot, int button) {
    ...
    public boolean globtype(char ch, KeyEvent ev) {
        if (ch == ' ') {
            if(ui.modflags() == 2){
				ui.menugrid.wdgmsg("act", "theTrav", "village");
			}else{
				int state = 1;
				if(Config.enableSpaceHearth) state = 0;
				if(ui.modflags() == state)
					ui.menugrid.wdgmsg("act", "theTrav", "hearth");
				else
					vc.toggle();
			}
            return (true);
    ...
    public int foldheight() {
        return (MainFrame.getInnerSize().y - c.y);
    }
