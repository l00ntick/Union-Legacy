...
public class HavenUtil{
	...
	public void craftItem(int All){
		...
		m_ui.make_window.wdgmsg("make", All);
	}
	...
	public void clickWorldObject(int button, Gob object){
		...
		//Coord sz = m_ui.mainview.sz;
		//Coord sc = new Coord((int) Math.round(Math.random() * 200 + sz.x / 2 - 100), (int) Math.round(Math.random() * 200 + sz.y / 2 - 100));
		Coord oc = object.getr();
					
		m_ui.mainview.wdgmsg("click", new Coord(400, 300), oc, button, 0, object.id, oc);
	}
	...
	public void sendAction(String str){
		String[] action = {str};
		m_ui.menugrid.wdgmsg("act", (Object[])action);
	}
	...
	public void setPlayerSpeed(int speed){
		if(stop) return;
		try{
			m_ui.speedget.setspeed(speed, true);
		}catch(Exception e){
	...
	public String slenError(){
		if(m_ui.slenhud.catchError != null)
			return m_ui.slenhud.catchError;
		else
			return "";
	}
	...
	public void useActionBar(int bar, int slot){
		if(bar == 0){
			if(m_ui.menugrid.digitbar.layout[slot] == null){
				return;
			}
			m_ui.menugrid.digitbar.layout[slot].use();
		}
		if(bar == 1){
			if(m_ui.menugrid.functionbar.layout[slot] == null){
				return;
			}
			m_ui.menugrid.functionbar.layout[slot].use();
		}
		if(bar == 2){
			if(m_ui.menugrid.numpadbar.layout[slot] == null){
				return;
			}
			m_ui.menugrid.numpadbar.layout[slot].use();
		}
		if(bar == 3){
			if(m_ui.menugrid.qwertypadbar.layout[slot] == null){
				return;
			}
			m_ui.menugrid.qwertypadbar.layout[slot].use();
		}
	}
	...
	public boolean findFlaskToolbar(int bar, int slot){
		...
		if(bar == 0){
			...
			barPad = m_ui.menugrid.digitbar;
		}else if(bar == 1){
			if(slot < 0 || slot > 11) return false;
			barPad = m_ui.menugrid.functionbar;
		}else if(bar == 2){
			if(slot < 0 || slot > 14) return false;
			barPad = m_ui.menugrid.numpadbar;
		}else if(bar == 3){
			if(slot < 0 || slot > 9) return false;
			barPad = m_ui.menugrid.qwertypadbar;
		}
	...
	public void setBeltSlot(int bar, int slot, Item i){
		...
		if(bar == 0){
			...
			//if(slot < 0 || slot > 9) return;
			//barPad = m_ui.menugrid.scriptBar;
		//}
		...
		m_ui.slenhud.wdgmsg("setbelt", s, 0);
	...
	public String getCraftName(){
		if( m_ui.make_window != null )
			return m_ui.make_window.title;
		
		return "N/A";
	}
	
	public boolean craftMenuReady(){
		return m_ui.make_window != null;
	}
	...
	public void flowerMenuSelect(String name){
		...
		for (int i = 0; i < m_ui.popupMenu.menuOptions.length; i++){
			if (m_ui.popupMenu.menuOptions[i].name.contains(name)) {
				m_ui.popupMenu.wdgmsg(m_ui.popupMenu, "cl", new Object[] { Integer.valueOf(m_ui.popupMenu.menuOptions[i].num) });
				break;
			}
		}
	}
	...
	public void flowerMenuSelectReverse(String name){
		if(!flowerMenuReady()) return;
		for (int i = (m_ui.popupMenu.menuOptions.length - 1); i >= 0 ; i--){
			if (m_ui.popupMenu.menuOptions[i].name.contains(name)) {
				m_ui.popupMenu.wdgmsg(m_ui.popupMenu, "cl", new Object[] { Integer.valueOf(m_ui.popupMenu.menuOptions[i].num) });
				break;
			}
	...
	public int pathSize(Coord c){
		pathing = true;
		PFrunning = true;
		PathWalker walk = new PathWalker(this, c, true);
		return walk.pathSize(c);
	}
	...
	public boolean barterTransfer(boolean extract){
		...
		//System.out.println("jBarterTransfer started");
	...
	public boolean barterTransfer(boolean extract, int shopbox){
		while(!windowOpen("Barter Stand") && !stop) wait(100);
		wait(500);
		...
		int space = getPlayerBagSpace();
		boolean filled = false;
		Shopbox box = ShopBoxList.get(shopbox);
			if(!extract){
				if(box.priceMatsInStand > 0){
					box.wdgmsg("buy", new Object[0]);
					filled = true;
