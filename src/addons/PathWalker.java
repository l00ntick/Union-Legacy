...
public class PathWalker extends Thread{
	...
	public PathWalker(HavenUtil h, Coord c, boolean checkOnly){
		m_util = h;
		pf = new PathFinder(m_util);
		m_gob = null;
		m_c = c;
		this.checkOnly = checkOnly;
		this.noBorders = !checkOnly;
	}
	
	public int pathSize(Coord c){
		return pathPoints(c).length;
	}
	
	public Coord[] pathPoints(Coord c){
		m_util.update();
		...
		getAllNegs(rects, null, start, end, waterPath, rePath, c);
		allRect.addAll(rects);
		allRect.addAll(m_critterGobs);
		path = pf.pathFind(allRect, start, end);
		if(path != null){
			pathCoord = coordConverter(path);
			m_util.PFrunning = false;
			m_util.update();
			return pathCoord.toArray(new Coord[pathCoord.size()]);
		} else {
			return new Coord[0];
		}
	}
		...
		while(/*rePath && */m_util.pathing){
			...
			if(path != null && m_util.pathing){
				...
				if(m_action != null && m_util.pathing && !rePath){
					m_util.m_ui.menugrid.wdgmsg("act", (Object[])m_action);
				...
				if(path != null && m_util.pathing){
					...
					if(m_util.pathing && !rePath){
						...
						if(m_action != null){
							...
							m_util.m_ui.mainview.wdgmsg("place", c, 1, 0);
						}else if(m_dropType == 3){
							m_util.clickWorld(3, c);
						/*}else if (g.resname().contains("gfx/arch/door-inn") || g.resname().contains("gfx/arch/cabin-door2") || g.resname().contains("gfx/arch/cabin-door2m")){
							Coord door
							m_util.clickWorld(1, c);
							m_util.clickWorldObject(3, g);*/
						}else{
	...
	void showPath(ArrayList<Coord> pathCoord, ArrayList<Rectangle> rects){
		...
		if(Config.pathfinderLine){
			...
			for(Coord c : draw){
				...
				m_drawer.addLine(c, f, 2, Color.GREEN);
				f = new Coord(c);
			}
		}
	}
	...
	void getAllNegs(ArrayList<Rectangle> allRect, Gob target, Point start, Point end, boolean waterPath, boolean repath){
		getAllNegs(allRect, target, start, end, waterPath, repath, null);
	}
	
	void getAllNegs(ArrayList<Rectangle> allRect, Gob target, Point start, Point end, boolean waterPath, boolean repath,  Coord mousepos){
		...
		synchronized(m_util.m_ui.mainview.glob.oc){
			for(Gob g : m_util.m_ui.mainview.glob.oc){
				...
				if( name.equals("gfx/terobjs/herbs/") ) continue;
		...
		ArrayList<Coord> filterdTiles = firstFilter2(player.getc(), waterPath, mousepos);
	...
	ArrayList<Coord> firstFilter2NoBorders(Coord player, boolean waterPath, Coord mousepos){
		//System.out.println("Tile size. " + tiles.size() );
		...
		/*Coord mouseposT = UI.instance.mainview.tilify(mousepos);
		Coord playerT = UI.instance.mainview.tilify(player);
		Coord gcNorm = UI.instance.mainview.tilify(gc.add(1,1).mul(1100));
		Coord myCoord = new Coord((playerT.x - gcNorm.x)/11,(playerT.y - gcNorm.y)/11);
		Coord mouseCoord = new Coord((mouseposT.x - gcNorm.x)/11,(mouseposT.y - gcNorm.y)/11);*/
				...
				for(int i = 0; i < 100; i++){
					for(int j = 0; j < 100; j++){
						if(gd != null){
							...
							/*while (gd == null) {
								m_util.wait(100);
								gd = m_util.m_ui.mainview.map.grids.get(gc.add(Mx,My) );
							}*/
			...
			//for(Coord c : tiles){
				...
				for(int i = 0; i < 4; i++){
					...
					if(waterPath && ((id >= 0 && id < 2))){
						...
						//Coord water = c.mul(11);
						//Coord waterCoord = new Coord((water.x - gcNorm.x)/11,(water.y - gcNorm.y)/11);
						//waterCoord = waterCoord.add(1,2);
						//System.out.println("water tile "+waterCoord);
						/*if (waterCoord.equals(mouseCoord.add(-1,-1)))System.out.println("water (-1,1) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(-1,0)))System.out.println("water (-1,0) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(-1,1)))System.out.println("water (-1,1) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(0,-1)))System.out.println("water (0,-1) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(0,0)))System.out.println("water (0,0) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(0,1)))System.out.println("water (0,1) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(1,-1)))System.out.println("water (1,-1) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(1,0)))System.out.println("water (1,0) "+waterCoord);
						if (waterCoord.equals(mouseCoord.add(1,1)))System.out.println("water (1,1) "+waterCoord);*/
	...
	ArrayList<Coord> firstFilter2(Coord player, boolean waterPath, Coord mousepos){
		if (noBorders) {
			return firstFilter2NoBorders(player, waterPath, mousepos);
		}
		//System.out.println("Tile size. " + tiles.size() );
		int[][] tileList = new int[300][300];
		ArrayList<Coord> filter = new ArrayList<Coord>();
		...
		Coord ulg = player.div(new Coord(1100, 1100)).add(-1,-1).mul(100);
		
		for(int Ny = -1; Ny < 301; Ny++){
			for(int Nx = -1; Nx < 301; Nx++){
				int test = -2;
				
				if (Nx >= 0 && Nx <= 299 && Ny >= 0 && Ny <= 299) {
					test = tileList[Nx][Ny];
				} else {
					if (mousepos != null && mousepos.div(1100).equals(ulg.add(Nx, Ny).div(100))) {
						test = -1;
					}
				}
