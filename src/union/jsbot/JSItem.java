...
public class JSItem {
	 ...
	 * @return true если предмет курьез (содержится в curios.conf)
	 */
	public boolean iscurio() {
		return (wdg().curios != null && wdg().curios.LP != 0);
	}
	...
	public int curioAttention() {
		if (wdg().curios != null && wdg().curios.LP != 0)
			return wdg().curios.weight;
		return -1;
	}
	...
	/**
	 * Возвращает базовое лп курьеза, работает с теми что описаны в curios.conf
	 * @return базовое лп курьеза или -1
	 */
	public double basecurioLP() {
		if (wdg().curios != null && wdg().curios.LP != 0) {
			return wdg().curios.LP;
		}
		return -1;
	}
	
	/**
	 * Возвращает лп курьеза зависящее от множителя и текущего ла, работает с теми что описаны в curios.conf
	 * @return текущее лп курьеза или -1
	 */
	public long currentcurioLP() {
		if (wdg().curios != null && wdg().curios.LP != 0) {
			return Math.round(wdg().curios.LP * wdg().qmult * UI.instance.wnd_char.getExpMode());
		}
		return -1;
	}
	...
	/**
	 * Возвращает полный тултип без имени русерса
	 * 
	 * @return полный тултип
	 */
	...
	/**
	 * Выбрасывает из инвентаря все предметы данного типа.
	 */
