...
public class APXUtils {
	...
	public static Coord _pf_real2map(Coord c) {
		...
		Coord l_til = MapView.tilify(UI.instance.mainview.myLastCoord);
	...
	public static Coord _pf_get_ltmc() {
		Coord rel = UI.instance.mainview.myLastCoord.div(_pf_tilesize);
	...
	public static void _pf_compute(int id_if_needed) {
		_pf_last_coord = UI.instance.mainview.myLastCoord;
		MapView mw = UI.instance.mainview;
		APXUtils._pf_clear();
		/* Tiles */
		for (int i = 0; i < _pf_dim; i++)
			for (int j = 0; j < _pf_dim; j++) {
	...
	public static Resource resScript6 = Resource
			.load("paginae/union/scripts/script6"); // folder
	...
	public static void addMenu() {
		...
		scriptRootNew = addResource("script_start", "Run Scripts",
				...
				resScript2, unionRoot, new MenuElemetUseListener(null) {
					public void use(int button) {
						if (button == 1) {
							JSScriptInfo.LoadAllScripts();
							JSScriptInfo.LoadAllScriptsToMenu();
						}
					}
				});
		...
		/*addResource("toggle_draw_pf_map", "Toggle PF Map", "Do it lol!", 'T',
				...
				new MenuElemetUseListener(null) {
					public void use(int button) {
						UI.instance.mainview.toggle_draw_pf();
					}
				});*/
	...
	public static void _add_stop_movement() {
		...
		if (UI.instance.mainview != null) {
			UI.instance.mainview.wdgmsg("click",
	...
	public static boolean isPathFree(Coord rc) {
		Coord my = UI.instance.mainview.myLastCoord;
		MapView mw = UI.instance.mainview;
