...
public class JSBotUtils {
	...
	public static Img hourGlass = null;
	...
	public static int tileType(int x, int y) {
		...
		mC = mC.add(tile);
		return UI.instance.mainview.getTileFix(mC);
	}
	
	public static int absTileType(int x, int y) {
		return UI.instance.mainview.getTileFix(new Coord(x, y));
	}
	...
	public static void doClick(int obj_id, int btn, int modflags) {
		...
		if (UI.instance.mainview != null) {
			...
			UI.instance.mainview.wdgmsg("click", new Coord(400, 300), oc, btn, modflags, obj_id,
					oc);
		}
	}
	...
	public static void dropObj(int m) {
		UI.instance.mainview.drop_thing(m);
	}
	...
	public static boolean isMoving() {
		Gob me = UI.instance.mainview.glob.oc.getgob(playerID);
	...
	public static boolean isMoving(int gob) {
		Gob me = UI.instance.mainview.glob.oc.getgob(gob);
		Moving m = me.getattr(Moving.class);
		if (m == null)
			return false;
		else
	...
	/* Other */
	public static boolean haveHourGlass() {
		return (hourGlass != null);
	}
	...
	public static int getProgress() {
		...
		String[] arr = prog.textureName.split("/");
		return (Integer.parseInt(arr[arr.length-1]));
	}
	
	public static void forfeit() {
		if (UI.instance.wnd_char != null) {
			UI.instance.wnd_char.wdgmsg("forfeit", 0);
		}
	}
	
	public static String get_wish(int i) {
		if (i > 2 || UI.instance.wnd_char.ancw == null || UI.instance.wnd_char.ancw.wishes[i] == null) {
			return "";
		} else {
			for (Widget it = UI.instance.wnd_char.ancw.wishes[i].child; it != null; it = it.next) {
	...
	public static int get_numen() {
		if (UI.instance.wnd_char.ancw == null || UI.instance.wnd_char.ancw.numen == null) {
			return -1;
		}
		return Integer.parseInt(UI.instance.wnd_char.ancw.numen.text);
	}
