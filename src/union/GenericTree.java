...
public class GenericTree {
	...
	public static class MenuElement {
		...
		public void RemoveFromMenu() {
			if (Glob.instance != null) {
				synchronized (Glob.instance.paginae) {
					Glob.instance.paginae.remove(res);
				}
			}
			RemoveListener();
		}
		
		public void SetListener(union.CustomMenu.MenuElemetUseListener lst) {
			//Kerri: fixed param by java tip (quick fix)
			if (UI.instance != null && action.length > 1) UI.instance.menugrid.listeners.put(action[1], lst);
		}
		
		public void RemoveListener() {
			if (UI.instance != null && action.length > 1) UI.instance.menugrid.listeners.remove(action[1]);
		}
	}
